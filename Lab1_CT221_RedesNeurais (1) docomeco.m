...
for i = 1:3
    ...
    netVector{i}.divideParam.trainRatio = 0.70;
    netVector{i}.divideParam.valRatio = 0.15;
    netVector{i}.divideParam.testRatio = 0.15;
end
...
for i = 1:3
    ...
    netVector{i}.layers{1}.dimensions = 15; % número de neurônios da camada interna
    netVector{i}.layers{1}.transferFcn = 'tansig'; % função de entrada na camada interna
    netVector{i}.layers{2}.transferFcn = 'purelin'; % função de saída da camada interna
    ...
    netVector{i}.trainFcn = 'trainlm'; % algoritmo de treinamento Levemberg-Marquadat

    netVector{i}.trainParam.epochs = 10^4; % número máximo de épocas de treinamento
    netVector{i}.trainParam.time = 480; % tempo máximo de treinamento
    netVector{i}.trainParam.lr = 0.15; % taxa de aprendizado
    netVector{i}.trainParam.min_grad = 10^-10; % valor mínimo do gradiente, como critério de parada
    netVector{i}.trainParam.max_fail = 1000; % número máximo de interações sem decaimento
