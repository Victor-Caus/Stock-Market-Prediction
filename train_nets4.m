...
for i = 1:3
    nets{i} = feedforwardnet(25,'traincgp'); % 1 camada internas
...
for j = indiceMaxTrein+1 : nAmostras
    for i = 1:3
...
auau = T_simu{1}(:,indiceMaxTrein+1:nAmostras);
y_pred_test = reshape(auau, [], 1);
y_test = close{1}((indiceMaxTrein+1)*10 -9:nAmostras*10)';
...
% Vale do rio doce (2)
y_pred_test = T_simu{2}(:,indiceMaxTrein+1:nAmostras);
y_pred_test = reshape(y_pred_test, [], 1);
y_test = close{2}((indiceMaxTrein+1)*10 -9:nAmostras*10)';
figure(5)
plot(y_test);
hold on;
plot(y_pred_test);
...
% Embraer (3)
y_pred_test = T_simu{3}(:,indiceMaxTrein+1:nAmostras);
y_pred_test = reshape(y_pred_test, [], 1);
y_test = close{3}((indiceMaxTrein+1)*10 -9:nAmostras*10)';
figure(6)
plot(y_test);
hold on;
plot(y_pred_test);
